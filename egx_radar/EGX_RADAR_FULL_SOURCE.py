"""Standalone consolidated source export for EGX Radar.

This file is auto-generated from the current project tree.
Each section starts with the original relative path.
"""
...
#==============================================================================
# FILE: advanced/portfolio_optimization.py
#==============================================================================
...
#==============================================================================
# FILE: advanced/risk_management.py
#==============================================================================
...
#==============================================================================
# FILE: backtest/engine_with_guards.py
#==============================================================================
...
#==============================================================================
# FILE: backtest/missed_trades.py
#==============================================================================
...
#==============================================================================
# FILE: backtest/tracking_dashboard.py
#==============================================================================
...
class K:
    ...
    PARTIAL_TP_PCT       = 0.07      # ← raised: take partial profits at +7% not +4%
    TRAILING_TRIGGER_PCT = 0.05      # ← raised: activate trail at +5% not +3%
    TRAILING_STOP_PCT    = 0.03      # ← widened: give winners room to breathe (3% trail)
    ...
    BT_MIN_SMARTRANK = 45.0          # ← HYBRID: lowered to 45 (MEDIUM tier threshold)
...
from egx_radar.ai.decision_engine import classify_trade
from egx_radar.ai.alpha_engine import compute_alpha_score
from egx_radar.ai.alpha_execution import build_alpha_trade
from egx_radar.ai.alpha_filter import passes_alpha_filter
from egx_radar.ai.news_live_feed import fetch_live_news
from egx_radar.ai.probability_engine import compute_probability
from egx_radar.ai.sentiment_engine import compute_sentiment
from egx_radar.config.settings import K, get_account_size, get_risk_per_trade
from egx_radar.core.accumulation import evaluate_accumulation_context
from egx_radar.core.indicators import (
    compute_atr,
...
log = logging.getLogger(__name__)
_NEWS_LIVE_ENABLED = os.getenv("EGX_ENABLE_LIVE_NEWS", "1").strip().lower() in {"1", "true", "yes", "on"}
_ALPHA_ENGINE_ENABLED = os.getenv("EGX_ENABLE_ALPHA_ENGINE", "1").strip().lower() in {"1", "true", "yes", "on"}
...
def _fetch_symbol_news(sym: str) -> List[dict]:
    """Fetch recent symbol-specific items from the live news cache/source layer."""
    if not _NEWS_LIVE_ENABLED:
        return []
    try:
    ...
    return [item for item in all_news if str(item.get("symbol", "")).upper() == symbol]
...
def _build_alpha_metadata(snapshot: Dict[str, object], symbol_news: List[dict]) -> None:
    """Attach early-opportunity alpha context without changing the main signal path."""
    snapshot["alpha_score"] = 0.0
    snapshot["alpha_trade"] = None
    snapshot["alpha_filter_reason"] = "alpha_disabled"

    if not _ALPHA_ENGINE_ENABLED:
        return
    if not symbol_news:
        snapshot["alpha_filter_reason"] = "no_news"
        return

    alpha_score = compute_alpha_score(symbol_news, snapshot)
    snapshot["alpha_score"] = alpha_score

    smart_rank = float(snapshot.get("smart_rank") or 0.0)
    if smart_rank >= 60.0:
        snapshot["alpha_filter_reason"] = "smart_rank_already_mature"
        return

    passed, reason = passes_alpha_filter(snapshot, alpha_score)
    snapshot["alpha_filter_reason"] = reason
    if not passed:
        return

    snapshot["alpha_trade"] = build_alpha_trade(snapshot, alpha_score)
...
def _enrich_ai_snapshot(snapshot: Dict[str, object]) -> Dict[str, object]:
    ...
    symbol_news = _fetch_symbol_news(str(snapshot.get("sym") or ""))
    sentiment_score = compute_sentiment(symbol_news, snapshot)
    snapshot["sentiment_score"] = sentiment_score
    _build_alpha_metadata(snapshot, symbol_news)

    probability = compute_probability(snapshot, sentiment_score=sentiment_score)
    snapshot["probability"] = probability
    snapshot["trade_class"] = classify_trade(probability)
...
#==============================================================================
# FILE: market_data/notifications.py
#==============================================================================
...
#==============================================================================
# FILE: tests/test_news_intelligence.py
#==============================================================================
...
#==============================================================================
# FILE: tools/paper_trading_tracker.py
#==============================================================================
