...
# Backtest helper — doesn't read from file
...
# FIX-2: K constants must NEVER be mutated at module level.
# Use a context manager so live scanner thresholds are always restored.
import contextlib

@contextlib.contextmanager
def _relaxed_dg_thresholds():
    """Temporarily relax DataGuard thresholds for backtest only. Always restores originals."""
    _orig = {
        "DG_VOLUME_ZSCORE_THRESHOLD":   K.DG_VOLUME_ZSCORE_THRESHOLD,
        "DG_CANDLE_SIZE_ATR_MULTIPLIER": K.DG_CANDLE_SIZE_ATR_MULTIPLIER,
        "DG_CONFIDENCE_FULL":           K.DG_CONFIDENCE_FULL,
        "DG_CONFIDENCE_DEGRADED":       K.DG_CONFIDENCE_DEGRADED,
    }
    try:
        K.DG_VOLUME_ZSCORE_THRESHOLD    = 5.0
        K.DG_CANDLE_SIZE_ATR_MULTIPLIER = 6.0
        K.DG_CONFIDENCE_FULL            = 55.0
        K.DG_CONFIDENCE_DEGRADED        = 30.0
        yield
    finally:
...
REGIME_MAP = {"MOMENTUM": "BULL", "ACCUMULATION": "BULL", "DISTRIBUTION": "BEAR", "NEUTRAL": "NEUTRAL"}
...
def _restore_state(backup: dict) -> None:
    with STATE._lock:
        ...
        STATE.signal_history = defaultdict(
            lambda: deque(maxlen=5),
            {k: deque(v, maxlen=5) for k, v in backup["signal_history"].items()},
        )
...
def _build_results_for_day(
    ...
    for yahoo, df in all_data.items():
        ...
        if df_slice.empty or len(df_slice) < K.MIN_BARS or df_slice.index[-1] != date:
            continue
            
        guard_stats["bars_processed"] = guard_stats.get("bars_processed", 0) + 1
        
        try:
            dg_result = _data_guard.evaluate(df_slice, sym)
        except Exception:
            from collections import namedtuple
            DGR = namedtuple("DGR", ["confidence_tier", "confidence"])
            dg_result = DGR(confidence_tier="FULL", confidence=100)
            
        if dg_result.confidence_tier == "REJECTED":
            guard_stats["DataGuard REJECTED"] = guard_stats.get("DataGuard REJECTED", 0) + 1
            continue
        # DEGRADED: remember we need to cap signal AFTER it's built.
        # Do NOT set max_signal_strength here — we must know the signal first.
        _dg_degraded = (dg_result.confidence_tier == "DEGRADED")
        if _dg_degraded:
            guard_stats["DataGuard DEGRADED"] = guard_stats.get("DataGuard DEGRADED", 0) + 1

        df_ta = df_slice.tail(250).copy()
        required = {"Close", "High", "Low", "Open"}
        if not required.issubset(set(df_ta.columns)): continue
        if "Volume" not in df_ta.columns: df_ta["Volume"] = 0.0

        close = df_ta["Close"].dropna()
        if len(close) < K.MIN_BARS: continue
        price = float(close.iloc[-1])
        if price < K.PRICE_FLOOR: continue
        if df_ta["Close"].iloc[-5:].nunique() <= 1: continue

        ema200_s = close.ewm(span=200, adjust=False).mean()
        ema50_s = close.ewm(span=50, adjust=False).mean()
        ema200, ema50 = last_val(ema200_s), last_val(ema50_s)
        pct_ema200 = safe_clamp(((price - ema200) / (ema200 + 1e-9)) * 100, -50.0, 50.0)
        
        raw_mom_s = close.pct_change(5).rolling(3).mean()
        momentum = float(raw_mom_s.iloc[-1]) * 100 if pd.notna(raw_mom_s.iloc[-1]) else 0.0
        momentum_prev = float(raw_mom_s.iloc[-2]) * 100 if len(raw_mom_s) >= 2 and pd.notna(raw_mom_s.iloc[-2]) else 0.0
        ...
        if adx_val >= 99.0 or rsi_val >= 99.0: continue
        
        avg_vol = float(df_ta["Volume"].rolling(20).mean().iloc[-1])
        if pd.isna(avg_vol) or (avg_vol > 0 and avg_vol < K.LOW_LIQ_FILTER): continue
        vol_ratio = safe_clamp(float(df_ta["Volume"].iloc[-1]) / (avg_vol + 1e-9), 0.0, 5.0)
        ...
        bar_h, bar_l, bar_o = float(df_ta["High"].iloc[-1]), float(df_ta["Low"].iloc[-1]), float(df_ta["Open"].iloc[-1])
        clv = safe_clamp(((price - bar_l) - (bar_h - price)) / (bar_h - bar_l + 1e-9), -1.0, 1.0) if bar_h > bar_l else 0.0
        
        ma10 = close.ewm(span=10, adjust=False).mean()
        trend_acc = safe_clamp(((ma10.iloc[-1] - ma10.iloc[-3]) - (ma10.iloc[-5] - ma10.iloc[-8])) / (price + 1e-9), -0.05, 0.05) if len(ma10) >= 8 else 0.0
        
        breakout = (price > float(df_ta["High"].iloc[-6:-1].max()) and vol_ratio > 1.3 and (abs(price - bar_o) / (bar_h - bar_l + 1e-9)) > K.FAKE_BREAK_BODY)
        
        phase = detect_phase(adx_val, rsi_val, vol_ratio, trend_acc, clv, momentum * min(1.4, max(0.6, adx_val / 25.0)), ema50, price, body_ratio=abs(price - bar_o) / (bar_h - bar_l + 1e-9))
        zone = detect_predictive_zone(rsi_val, momentum * min(1.4, max(0.6, adx_val / 25.0)), pct_ema200, vol_ratio)

        # BUILD SIGNAL
        _, _, brain_score_req = STATE.snapshot_brain()
        signal, tag, tech_score = build_signal(price, ema200, ema50, adx_val, rsi_val, momentum * min(1.4, max(0.6, adx_val / 25.0)), vol_ratio, breakout, pct_ema200, phase, brain_score_req=brain_score_req)
        
        # --- DEGRADED cap: ONLY downgrade existing strong signals ---
        # NEVER generate new entries for bars that had no signal.
        if _dg_degraded:
            if tag in ("buy", "ultra", "early"):
                # downgrade strong → watch
                tag = "watch"; signal = "⏳ DEGRADED"
            elif tag in ("watch", "probe"):
                pass  # already weak, keep as is
            else:
                # No signal existed → DEGRADED should not create one
                continue
            
        try:
            mg_result = _momentum_guard.evaluate(sym, momentum, momentum_prev, adx_val, vol_ratio)
        except Exception:
            class DummyMG: defensive_mode = False; fatigue_mode = False; position_scale = 1.0; effective_rank_threshold_boost = 0
            mg_result = DummyMG()
        
        if mg_result.defensive_mode: guard_stats["MomentumGuard defensive"] += 1
        if mg_result.fatigue_mode: guard_stats["MomentumGuard fatigue"] += 1
        
        pos_scale = 1.0 * mg_result.position_scale * (am_result.position_scale if am_result else 1.0)
        rank_boost = mg_result.effective_rank_threshold_boost + (am_result.rank_threshold_boost if am_result else 0)

        # From here, standard logic continues
        ...
        silent = (45 <= rsi_val <= 60 and trend_acc > 0 and clv > 0.5 and vol_ratio < 1.3 and (abs(price - bar_o) / (bar_h - bar_l + 1e-9)) < 0.35)
        hunter = adx_val > 18 and 40 <= rsi_val <= 65 and vol_ratio > 1.25 and trend_acc > 0
        whale_flag = "🐳 Whale" if (vol_zscore > K.WHALE_ZSCORE_THRESH and clv > K.WHALE_CLV_SIGNAL and vol_ratio > K.WHALE_VOL_THRESH) else ""
        ...
        grav_lbl, grav_n = score_gravity(clv, trend_acc, vol_ratio, rsi_val, momentum)
        ...
        if sector in sec_quant: sec_quant[sector].append(quantum_n)
        
        results.append({
            "sym": sym, "sector": sector, "price": price, "adx": adx_val, "rsi": rsi_val, "momentum": momentum,
            "adaptive_mom": momentum * min(1.4, max(0.6, adx_val / 25.0)), "pct_ema200": pct_ema200, "vol_ratio": vol_ratio,
            "tech_score": tech_score, "signal": signal, "tag": tag, "phase": phase, "trend_acc": trend_acc,
            "breakout": breakout, "clv": clv, "whale": whale_flag, "hunter": "🎯 Hunter" if hunter else "",
            "silent": silent, "expansion": expansion, "fake_expansion": fake_exp,
            "cpi": iet_n, "iet": iet_n, "whale_score": whale_n, "anticipation": (0.0 if tag == "sell" else ant_n),
            "quantum": quantum_n, "gravity_label": grav_lbl, "gravity_score": grav_n, "zone": zone,
            "hidden": 0.0, "ema_cross": ema_cross, "vol_div": vol_div, "mom_arrow": mom_arr, "atr": atr,
            "atr_risk": atr_label, "atr_pct_rank": atr_pct_rank, "vwap_dist": vwap_dist, "vol_zscore": vol_zscore,
            "smart_rank": 0.0, "confidence": 0.0, "leader": False, "plan": None, "inst_conf": "—",
            "signal_dir": "⏸️ NEUTRAL", "signal_reason": "", "signal_display": "", "guard_reason": "",
            "rank_threshold_boost": rank_boost, "pos_scale": pos_scale
        })
    ...
    for r in results:
        ...
        rank_boost_val = r.get("rank_threshold_boost", 0)
        ...
        r["inst_conf"] = institutional_confidence(
            ...
            dg_tier=dg_result.confidence_tier if dg_result else "",
            mg_passed=not (mg_result and mg_result.defensive_mode),
        )
...
def run_backtest(
...
) -> Tuple[List[dict], List[Tuple[str, float]], Dict[str, Any], Dict[str, int]]:
    """
    ...
    with _relaxed_dg_thresholds():
        ...
        try:
            ...
            _dg = DataGuard(); _mg = MomentumGuard(); _am = AlphaMonitor(); _pm = PositionManagerBT()
            guard_stats = {
                "DataGuard REJECTED": 0, "DataGuard DEGRADED": 0,
                "MomentumGuard defensive": 0, "MomentumGuard fatigue": 0,
                "AlphaMonitor L1": 0, "AlphaMonitor L2": 0, "AlphaMonitor L3 (pause)": 0,
                "PositionManager addon approved": 0, "bars_processed": 0
            }

            for day_idx, date in enumerate(trading_days):
                # Step 3a debug: confirm AlphaMonitor receives data
                print(f"[AlphaMonitor] evaluate_from_trades called with {len(closed_trades)} trades (day {day_idx+1}/{len(trading_days)})", flush=True)
                try: am_res = _am.evaluate_from_trades(closed_trades)
                except Exception:
                    class DummyAM: warning_level=0; position_scale=1.0; rank_threshold_boost=0; pause_new_entries=False
                    am_res = DummyAM()
                
                if am_res.warning_level == 1: guard_stats["AlphaMonitor L1"] += 1
                elif am_res.warning_level == 2: guard_stats["AlphaMonitor L2"] += 1
                elif am_res.warning_level == 3: guard_stats["AlphaMonitor L3 (pause)"] += 1

                if progress_callback and day_idx % 5 == 0:
                    progress_callback(f"Scanning {date.strftime('%Y-%m-%d')}... ({day_idx+1}/{len(trading_days)})")
            
                results, sector_strength, regime_raw = _build_results_for_day(
                    all_data, date, yahoo_to_sym, _dg, _mg, am_res, guard_stats
                )
                regime_tag = REGIME_MAP.get(regime_raw, "NEUTRAL")
                results_by_sym = {r["sym"]: r for r in results}

                date_str = date.strftime("%Y-%m-%d")
                still_open: List[dict] = []
                for t in open_trades:
                    y = SYMBOLS.get(t["sym"])
                    if not y or y not in all_data: still_open.append(t); continue
                    df = all_data[y]
                    if date not in df.index: t["bars_held"] += 1; still_open.append(t); continue
                    ...
                    exit_price = c
                    if t["is_short"]:
                        if l <= t["target"]: outcome = "WIN"; exit_price = t["target"]
                        elif h >= t["stop"]: outcome = "LOSS"; exit_price = t["stop"]
                        elif t["bars_held"] >= max_bars: outcome = "EXIT"
                    else:
                        if h >= t["target"]: outcome = "WIN"; exit_price = t["target"]
                        elif l <= t["stop"]: outcome = "LOSS"; exit_price = t["stop"]
                        elif t["bars_held"] >= max_bars: outcome = "EXIT"

                    if outcome is None:
                        sr = results_by_sym.get(t["sym"])
                        if sr:
                            try:
                                t["status"] = "OPEN"
                                ad = _pm.evaluate_addon(t["sym"], c, sr["adx"], sr["momentum"], sr["smart_rank"], sr.get("rank_threshold_boost", 0), sr["vol_div"])
                                if getattr(ad, "approved", False):
                                    guard_stats["PositionManager addon approved"] += 1
                                    osz, asz = t.get("size", 1.0), getattr(ad, "add_on_r", 0.0)
                                    t["entry"] = (t["entry"] * osz + c * asz) / (osz + asz)
                                    t["size"] = osz + asz
                                    if not t["is_short"]: t["stop"] = getattr(ad, "new_stop_addon", t["stop"])
                                    t["addon_count"] = t.get("addon_count", 0) + 1
                            except Exception: pass
                        still_open.append(t)
                    else:
                        p = (t["entry"] - exit_price) / t["entry"] * 100 if t["is_short"] else (exit_price - t["entry"]) / t["entry"] * 100
                        p -= K.BT_TOTAL_COST_PCT * 100   # FIX-4: round-trip cost
                        # FIX-B: Skip NaN trades entirely
                        if not np.isfinite(p):
                            log.warning(
                                "FIX-B: NaN pnl for %s exit=%s — trade skipped",
                                t.get("sym"), exit_price,
                            )
                            still_open.append(t)
                            continue
                        equity *= (1 + 0.20 * p / 100.0)
                        if outcome == "LOSS": _mg.record_loss_event(t["sym"])
                        _mg.record_breakout_result(t["sym"], p > 0, date)
                        cl = {**t, "exit_date": date_str, "exit": exit_price, "pnl_pct": round(p, 2), "status": "CLOSED", "outcome": outcome}
                        closed_trades.append(cl)
                open_trades = still_open


                # Then: open new trades from today's guarded signals
                guarded_list, _, guard_exp, _ = compute_portfolio_guard(results)
                ...
                for gr in allowed:
                    ...
                    target = plan["target"]
                    is_short = r["tag"] == "sell"
                    if am_res and getattr(am_res, "pause_new_entries", False): continue
                    if is_short:
                        entry = r["price"]
                    ...
                    open_trades.append({
                        ...
                        "bars_held": 0,
                        "is_short": is_short,
                        "size": r["plan"].get("size", 1.0), "setup_type": r["tag"], "status": "OPEN",
            ...
            return closed_trades, equity_curve, {
                ...
                "max_bars": max_bars, "max_concurrent_trades": max_concurrent_trades,
            }, guard_stats
        finally:
